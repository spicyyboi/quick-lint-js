...
class basic_text_error_formatter : public error_formatter<basic_text_error_formatter> {
 public:
...
#undef QLJS_ERROR_TYPE
  ...
  void report_fatal_error_unimplemented_token(
      const char *, int , const char *,
      token_type, const char8 *) override {
  }
...
void basic_text_error_formatter::write_after_message(severity, const source_code_span &) {
...
class test_translation : public ::testing::Test {
  public:
    void TearDown() override {
      initialize_translations_from_locale(QLJS_LOCALE_DIR, "");
    }
...
TEST_F(test_translation, c_language_does_not_translate_diagnostics) {
  initialize_translations_from_locale(QLJS_LOCALE_DIR, "C");
  this->reporter.report(error_unexpected_hash_character{
      this->dummy_span()});
